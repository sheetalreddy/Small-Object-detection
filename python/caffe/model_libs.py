...
def DeConvBNLayer(net, from_layer, out_layer, use_bn, use_relu, num_output,
  ...
  [stride_h, stride_w] = UnpackVariable(stride, 2)
  if kernel_h == kernel_w:
    net[conv_name] = L.Deconvolution(net[from_layer], convolution_param=dict(num_output=num_output,kernel_size=kernel_h, pad=pad_h, dilation=[1],stride=stride_h,weight_filler= dict(type='gaussian', std=0.01),bias_term=False), **kwargs)
    
#  convolution_param=dict(num_output=num_output_dec, kernel_size=kernel_size, stride=stride, pad=[0], dilation=[1], group=num_output_dec,
...
def Concat(net , from_layer , out_layer , concat_layer):
   
    conv_name = '{}{}{}'.format('', out_layer, '')
    net[conv_name] = L.Concat(net[concat_layer],net[from_layer])
...
def CreateMultiBoxHead(net, data_layer="data", num_classes=[], from_layers=[],
    ...
    num_priors_per_location_conv4_3=0
    num = len(from_layers)
    priorbox_layers = []
    loc_layers = []
    conf_layers = []
    ...
    for i in range(0, num):
        ...
        if len(steps) > i:
            step = steps[i]
        if i==0:
              num_priors_per_location_conv4_3=num_priors_per_location
...
#deconv and concat confidence layers for context
    context_layers=[]
    kernel=[0,20,29,34]
    for i in range(1,4):
       concat_name = "{}_mbox_conf{}".format(from_layers[i], conf_postfix)
       concat_name_out = "{}_mbox_conf_out{}".format(from_layers[i], conf_postfix)
       DeConvBNLayer( net, concat_name, concat_name_out, use_batchnorm, True, 32, kernel[i], 0, 1,  lr_mult=lr_mult)
       context_layers.append(net[concat_name_out])
    norm_name = "{}_norm".format(from_layers[0])
    context_layers.append(net[norm_name])
    concat_context_name= "{}_context".format(from_layers[0])
    net[concat_context_name]=L.Concat(*context_layers,axis=1)
#create confidence layer for conv4_3
    name = "{}_mbox_conf{}".format(norm_name, conf_postfix)
    num_conf_output = num_priors_per_location_conv4_3 * num_classes
    print(num_conf_output)
    ConvBNLayer(net,concat_context_name, name, use_bn=use_batchnorm, use_relu=False, lr_mult=lr_mult,
            num_output=num_conf_output, kernel_size=kernel_size, pad=pad, stride=1, **bn_param)
...
#for loop for flattening the layers 
    for i in range(0, num):
        from_layer = from_layers[i]
...
#    if num==1:
    name = "mbox_loc"
   # loc_layers.append(net['mbox_loc_1'])
    net[name] = L.Concat(*loc_layers, axis=1)
    mbox_layers.append(net[name])
    name = "mbox_conf"
   # conf_layers.append(net['mbox_conf_1'])
    net[name] = L.Concat(*conf_layers,axis=1)
    mbox_layers.append(net[name])
    name = "mbox_priorbox"
    #priorbox_layers.append(net['mbox_priorbox_1'])
    net[name] = L.Concat(*priorbox_layers,axis=2)
    mbox_layers.append(net[name])
    if use_objectness:
	name = "mbox_objectness"
     #   priorbox_layers.append(net['mbox_objectness_1'])
...
def CreateMultiBoxHead_old(net, data_layer="data", num_classes=[], from_layers=[],
        use_objectness=False, normalizations=[], use_batchnorm=True, lr_mult=1,
        use_scale=True, min_sizes=[], max_sizes=[], prior_variance = [0.1],
        aspect_ratios=[], steps=[], img_height=0, img_width=0, share_location=True,
        flip=True, clip=True, offset=0.5, inter_layer_depth=[], kernel_size=1, pad=0,
